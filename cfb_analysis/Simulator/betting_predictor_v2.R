...
data <- read.csv(file = 'data\\2016\\perplaydata.csv', sep=',', header=T)
...
working[working$OPP.SCORE == 0,]$OPP.SCORE <- 1
#working[working$SCORE == 0,]$SCORE <- 1
...
OppSoS <- SoS
colnames(OppSoS)[colnames(OppSoS) == "Points.For"] <- "Opp.Points.For" 
colnames(OppSoS)[colnames(OppSoS) == "Conf.Points.For"] <- "Opp.Conf.Points.For" 
colnames(OppSoS)[colnames(OppSoS) == "Points.Against"] <- "Opp.Points.Against" 
colnames(OppSoS)[colnames(OppSoS) == "Conf.Points.Against"] <- "Opp.Conf.Points.Against" 
...
setup.predict <- function(teamA, teamB, runCount)
{
  ...
  #REMOVE WHEN DATA FINALLY SUFFICIENT
...
neutral.predict <- function(teamA, teamB, spread, over, runCount)
{
  ...
  box.score$Spread.Wins <- length(games[games$Home.Score+spread > (games$Away.Score),]$Home.Score)
  away.box$Spread.Wins <- length(games[games$Home.Score+spread < (games$Away.Score),]$Home.Score)
  
  box.score$Over.Wins <- length(games[(games$Home.Score + games$Away.Score) > over,]$Home.Score)
  away.box$Over.Wins <- length(games[(games$Home.Score + games$Away.Score) < over,]$Home.Score)
  
  names(box.score) <- c("Team", "Pass Yards", "Rush Yards", "Penalty Yards", "Turnovers", "Score", "WINS", "Spread Wins", "Over/Under")
  names(away.box) <- c("Team", "Pass Yards", "Rush Yards", "Penalty Yards", "Turnovers", "Score", "WINS", "Spread Wins", "Over/Under")
...
predict <- function(teamA, teamB, spread, over, runCount)
{
