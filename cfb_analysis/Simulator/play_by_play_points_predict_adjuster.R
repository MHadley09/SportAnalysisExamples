...
neutral.game <- function(row)
{
  ...
  allowedOff <- 2 - fit
  home.points <- (allowedOff*home.points + fit*away.points.allowed )/2
  away.points <- (allowedOff*away.points + fit*home.points.allowed )/2
...
game <- function(row)
{
  ...
  game[["Home Score"]] <- 1.5+home.SoS.adjust*home.points
  game[["Away Score"]] <- max(0,-1.5+away.SoS.adjust*away.points)
  return(game)
}


neutral.predict <- function(teamA, teamB, runCount, fittingValue)
{
