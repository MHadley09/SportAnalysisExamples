neutral.predict <- function(homeName, awayName, runCount){
  ...
  for(i in 1:length(homeOpps$OPP))
  {
    ...
    SoSOff <- sqrt(SoS[SoS$TEAM == oppName,]$Adjusted.SoS / maxSoS )
    DSoSOff <- sqrt(maxSoS / SoS[SoS$TEAM == oppName,]$Adjusted.SoS)
    ...
    homePassOYards.Past[i] <-  SoSOff*homeOpps[i,]$P.YARDS.FOR
    homePassDYards.Past[i] <- DSoSOff*homeOpps[i,]$P.YARDS.AGAINST
    homeRunOYards.Past[i] <- SoSOff*homeOpps[i,]$R.YARDS.F
    homeRunDYards.Past[i] <- DSoSOff*homeOpps[i,]$R.YARDS.AGAINST
    homePF.Past[i] <-  SoSOff*homeOpps[i,]$POINTS.FOR
  ...
  for(i in 1:length(awayOpps$OPP))
  {
    ...
    awayPassOYards.Past[i] <- SoSOff*awayOpps[i,]$P.YARDS.FOR
    awayPassDYards.Past[i] <- DSoSOff*awayOpps[i,]$P.YARDS.AGAINST
    awayRunOYards.Past[i] <- SoSOff*awayOpps[i,]$R.YARDS.F
    awayRunDYards.Past[i] <- DSoSOff*awayOpps[i,]$R.YARDS.AGAINST
    awayPF.Past[i] <- SoSOff*awayOpps[i,]$POINTS.FOR
