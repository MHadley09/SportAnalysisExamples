All.SoS <- function(){
  ...
  for(i in 1:nrow(schedule))
  {
    ...
    records[records$TEAM == teamA,]$Points.For <- records[records$TEAM == teamA,]$Points.For + ( game$Home.Score^2.37)/(( game$Home.Score^2.37)+(game$Visitor.Score^2.37))
    records[records$TEAM == teamA,]$Points.Against <- records[records$TEAM == teamA,]$Points.Against + ( game$Visitor.Score^2.37)/(( game$Home.Score^2.37)+(game$Visitor.Score^2.37))
    
    records[records$TEAM == teamB,]$Points.For <- records[records$TEAM == teamB,]$Points.For +( game$Visitor.Score^2.37)/(( game$Home.Score^2.37)+(game$Visitor.Score^2.37))
    records[records$TEAM == teamB,]$Points.Against <- records[records$TEAM == teamB,]$Points.Against + ( game$Home.Score^2.37)/(( game$Home.Score^2.37)+(game$Visitor.Score^2.37))
    ...
    if(confA == confB)
    {
      records[records$TEAM == teamA,]$Conf.Points.For <- records[records$TEAM == teamA,]$Conf.Points.For + ( game$Home.Score^2.37)/(( game$Home.Score^2.37)+(game$Visitor.Score^2.37))
      records[records$TEAM == teamA,]$Conf.Points.Against <- records[records$TEAM == teamA,]$Conf.Points.Against +( game$Visitor.Score^2.37)/(( game$Home.Score^2.37)+(game$Visitor.Score^2.37))
      
      records[records$TEAM == teamB,]$Conf.Points.For <- records[records$TEAM == teamB,]$Conf.Points.For +  ( game$Home.Score^2.37)/(( game$Home.Score^2.37)+(game$Visitor.Score^2.37))
      records[records$TEAM == teamB,]$Conf.Points.Against <- records[records$TEAM == teamB,]$Conf.Points.Against +( game$Visitor.Score^2.37)/(( game$Home.Score^2.37)+(game$Visitor.Score^2.37))
    }
  }
  
  records$CONF.W <- records$Conf.Points.For
  records$CONF.L <- records$Conf.Points.Against
  records$OVERALL.W <- records$Points.For
  records$OVERALL.L <- records$Points.Against
