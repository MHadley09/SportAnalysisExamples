...
betting <- read.csv(file = 'data\\2016\\schedule.csv', sep=',', header=T)
...
b <- betting[betting$TeamB %in% conf[conf$Conference != "FCS",]$TEAM,]
b <- b[b$TeamA %in% conf[conf$Conference != "FCS",]$TEAM,]
conf[!conf$TEAM %in% rec$TEAM,]$TEAM

write.table(b, 'data\\2016\\schedule.csv', row.names=FALSE, sep=",") 
