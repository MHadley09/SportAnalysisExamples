...
data <- read.csv(file = 'data\\2005\\perplaydata.csv', sep=',', header=T)

data$Visitor.Team <- trim(data$Visitor.Team)
data$Home.Team <- trim(data$Home.Team)

FBS.Team <- read.csv(file = 'data\\2005\\records.csv', sep=',', header=T)
Conferences <- read.csv(file = 'data\\2005\\conferenceWithFCS.csv', sep=',', header=T)
...
data <- read.csv(file = 'data\\2008\\perplaydata.csv', sep=',', header=T)

data$Visitor.Team <- trim(data$Visitor.Team)
data$Home.Team <- trim(data$Home.Team)

FBS.Team <- read.csv(file = 'data\\2008\\records.csv', sep=',', header=T)
Conferences <- read.csv(file = 'data\\2008\\conferenceWithFCS.csv', sep=',', header=T)
...
game <- function(row)
{
  ...
  game[["Home Score"]] <- max(0, (1.5+(home.SoS.adjust*home.points)+ (0*turnover.margin) +
                                    (1.75*yardage.margin) + (1.75*penalty.margin)))
                                  
                               
  game[["Away Score"]] <- max(0, (-1.5+(away.SoS.adjust*away.points)+ (0*turnover.margin) +
                                    (-1.75*yardage.margin) + (-1.75*penalty.margin)))
  return(game)
}
