...
calculate.rankings <- function()
{
  ...
  data <- rbind(data, data.frame(Team = data$Opp, Score = data$Opp.Score, 
                                 Opp = data$Team, Opp.Score = data$Score, Neutral=paste(schedule$Neutral, "A", sep="")))
  ...
  for(j in 1:10)
  {
    for(i in 1:length(rankings$Team))
    {
      ...
      site <- ifelse(data[data$Team==as.character(rankings[i,]$Team),]$Neutral %in% c("N", "NE", "A", "NA", "EA", "NEA"), 0, 1)
  ...
  data <- data[!data$Neutral %in% c("N", "NE", "A", "NA", "EA", "NEA"),]
  ...
  for(i in 1:length(rankings$Team))
  {
    ...
    rankings[i,]$Home.Score <- optimize(f=func, interval=c(0,350))$minimum
  }    
