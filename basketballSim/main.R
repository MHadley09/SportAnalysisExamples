...
select.simulator <- function()
{
  ...
  options[1,] <-  c("B", "Run standard bell simulator", "bball_bell_predictor.R")
...
homeAwayBet <- function()
{
  ...
  runs <- readinteger("How many games should we simulate:\t")
  print(predict(homeTeam,awayTeam, spread, runs))
  print("")
}
...
neutralSiteBet <- function()
{
  ...
  over <- read.double("What is the over?\t")
  runs <- readinteger("How many games should we simulate:\t")
  print(neutral.predict(homeTeam,awayTeam, spread, runs))
  print("")
}
...
simulator <- function(){
  ...
  options[1,] <- c("(S)", "Simulate a match up")
  options[2,] <- c("(B)", "Generate betting odds")
  options[3,] <- c("(Q)", "Quit Application")
  while(T)
  {
