...
#     homeAdjOff[i] <- (homeOpps[i,]$Score - oppMean$Opp.Score)/oppStd$Opp.Score
#     homeAdjDef[i] <- (homeOpps[i,]$Opp.Score - oppMean$Score)/oppStd$Score
#   }
...
#     awayAdjOff[i] <- (homeOpps[i,]$Score - oppMean$Opp.Score)/oppStd$Opp.Score
#     awayAdjDef[i] <- (homeOpps[i,]$Opp.Score - oppMean$Score)/oppStd$Score
#   }
#  
...
#   
#   home.score <- ((1.1*(home.Mean$Score + (home.Sd$Score*awayDefense))) 
#                  + (.9*(away.Mean$Opp.Score + away.Sd$Opp.Score*homeOffense)))/2
#   away.score <- ((1.1*(away.Mean$Score + (away.Sd$Score*homeDefense))) 
#                  + (.9*(home.Mean$Opp.Score + home.Sd$Opp.Score*awayOffense)))/2
#   
...
predict <- function(homeName, awayName, runCount)
{
  ...
  home.score <- ((1.1*(home.Mean$Score*awayDefense)) 
