...
neutral.predict <- function(homeName, awayName, spread, overunder, runCount)
{
  ...
  homeScore <- rnorm(n=runCount,mean=home.Mean$Score,sd=homeAdjOff)
  awayOffense <- rnorm(n=runCount,mean=mean(home.Mean, na.rm = T),sd=sd(awayAdjOff, na.rm = T))
  homeDefense <- rnorm(n=runCount,mean=mean(homeAdjDef, na.rm = T),sd=sd(homeAdjDef, na.rm = T))
  awayDefense <- rnorm(n=runCount,mean=mean(awayAdjDef, na.rm = T),sd=sd(awayAdjDef, na.rm = T))
  
  home.score <- ((1.1*(homeScore*awayDefense)) 
