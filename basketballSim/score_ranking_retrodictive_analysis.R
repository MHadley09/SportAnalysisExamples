...
fit.func <- function(adv)
{
  ...
  gamechecker[!gamechecker$Neutral %in% c("N","NE"),]$Home.Value <- (1.028155*gamechecker[!gamechecker$Neutral %in% c("N","NE"),]$Home.Value)-.02687783
...
best <- optimize(f = fit.func, interval = c(-.25,.25), maximum=T)