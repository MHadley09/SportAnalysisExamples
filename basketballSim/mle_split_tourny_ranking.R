...
# {
#   x <- pmin(pmax(.05, scoreA - scoreB),2)
...
game.value <- function(scoreA, scoreB)
{
  x <- pmin(pmax(.5, (scoreA / scoreB)^v),2)
  return(.5 + (.692494*log(x)))
}

tournament.game.value <- function(scoreA, scoreB)
{
  return(ifelse(scoreA > scoreB, .95, .05))
}
...
calculate.rankings <- function()
{
  ...
  schedule <- schedule[schedule$Tournament == "T",]
  schedule <- schedule[schedule$Home.Team %in% as.character(leagues[leagues$League == "NCAA" & leagues$Division == "I",]$Team),]
  ...
  rankings$Tournament.Score <- rankings$Score
  ...
  for(j in 1:3)
  {
    for(i in 1:length(rankings$Team))
    {
      ...
      scores <- data[data$Team==as.character(rankings[i,]$Team),]$Score
      oppScores <-  data[data$Team==as.character(rankings[i,]$Team),]$Opp.Score
      gameScores <- tournament.game.value(scores, oppScores)
      oppScore <- rankings[rankings$Team %in% opps,]
      if(length(opps) > 0){  
        games <- merge(data.frame(Opp=opps, Game.Value=gameScores), 
                     data.frame(Opp=oppScore$Team, Opp.Value=oppScore$Previous.Score), by="Opp")
        ...
        rankings[i,]$Tournament.Score <- optimize(f=func, interval=c(0,3500), maximum=T)$maximum    
      }      
    }
    rankings$Previous.Score <- rankings$Tournament.Score
  }
  
  rankings$Final.Score <- (rankings$Tournament.Score + rankings$Score)/2  
  rankings$Rank <- (1 + length(rankings$Final.Score) - rank(rankings$Final.Score))
